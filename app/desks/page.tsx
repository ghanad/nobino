...
import { MyDeskReservations } from "@/app/desks/my-desk-reservations";
...
export default async function DesksPage({ searchParams }: Props) {
  ...
  const now = new Date();
  const date = parseJalaliDateParam(params?.date) ?? startOfLocalDay(now);
  const dateParam = formatJalaliDateParam(date);
  const [buildings, lunchBuildings, lunchReservation, lunchDayState, myUpcomingReservations] = await Promise.all([
  ...
  return <div className="grid gap-3" dir="rtl">
    <PageHeader title="رزرو میز کار" subtitle="دفتر، تاریخ و زمان حضور خود را انتخاب کنید و میز مناسب را از روی نقشه رزرو کنید." />
    ...
    {!building ? <p className="rounded-lg border bg-card p-5 text-sm text-muted-foreground">هنوز دفتر فعالی تعریف نشده است.</p> :
      <DeskReservationForm
        ...
        canModify={Boolean(myReservation && myReservation.endAt > now)}
        isStarted={Boolean(myReservation && myReservation.startAt <= now)}
        lunchAvailability={{
    ...
    {building ? (
      <MyDeskReservations
        buildingId={building.id}
        date={dateParam}
        reservations={myUpcomingReservations.map((reservation) => ({
          buildingName: reservation.desk.building.name,
