...
import { AlertTriangle, CheckCircle2, CircleSlash2, Radio } from "lucide-react";
...
export default async function AdminBalePage() {
  ...
  const [users, botState, deliveryCounts, latestFailure, latestSuccess] =
    await Promise.all([
      db.user.findMany({
  ...
  return (
    <div className="grid gap-6 text-right" dir="rtl">
      <PageHeader
        subtitle="وضعیت اتصال کاربران و سلامت ارسال اعلان‌ها"
        title="پیام‌رسان بله"
      />
      ...
      <section className="grid gap-4 rounded-lg border bg-card p-5 text-card-foreground">
        ...
        <div className="flex items-start gap-2 rounded-md bg-sky-50 p-3 text-xs leading-5 text-sky-900">
          <Radio className="mt-0.5 h-4 w-4 shrink-0" />
          متصل بودن فقط به معنی ثبت گفت‌وگوی کاربر است. معیار سلامت واقعی، تازه بودن همگام‌سازی و نبود پیام ناموفق است.
        </div>
      </section>
    </div>
  );
