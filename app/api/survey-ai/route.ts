...
export async function POST(request: Request) { try { const user = await requireCurrentUser(); const parsed = schema.safeParse(await request.json()); if (!parsed.success) return NextResponse.json({ error: "درخواست نامعتبر است." }, { status: 400 }); return NextResponse.json(await createSurveyAiProposal({ actorUserId: user.id, ...parsed.data })); } catch (error) { if (error instanceof SurveyServiceError) return NextResponse.json({ error: error.message }, { status: 409 }); return NextResponse.json({ error: "دریافت پیشنهاد هوش مصنوعی ناموفق بود." }, { status: 502 }); } }
