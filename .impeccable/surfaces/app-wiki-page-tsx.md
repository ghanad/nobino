...
- Administration: admins can open `/admin/wiki-ai` from the wiki header to enable or disable the assistant, configure the allowlisted OpenAI-compatible base URL, model, timeout, and output-token limit, test the connection/model listing, and receive explicit success or failure feedback; settings changes remain audited.
